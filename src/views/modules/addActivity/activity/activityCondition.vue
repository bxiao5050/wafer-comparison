<template>
  <div>
    <template v-for="item in config">
      ...
      >
        ...
        <template v-if="!checkDisabled(listSelected,item)">
          ...
          <div class="row-item">
            ...
            <el-select
              v-if="item.loginType!=3"
              v-model="item.activeLoginType"
              style="width:110px"
            >
              <el-option
                :value="1"
      ...
      <div v-if="item.conditionId=='ce59f4f3-75c9-472b-bfe7-0280405df206'">
        ...
        <template v-if="!checkDisabled(listSelected,item)">
          <a
            ...
            @click="downTemplate(item.conditionId)"
          >模板下载</a>
          ...
          <el-upload-pack
            :item="item"
            @changePercent="$emit('changePercent')"
            @responseResource="fixedResource(item, arguments[0])"
          ></el-upload-pack>
      ...
      <!--投票-->
      <div v-if="item.conditionId=='a4ded85d-69e3-4132-90d3-59dadc98312f'">
        <el-checkbox
          class="radio"
          v-model="listSelected"
          :label="item.conditionId"
        ...
        <template v-if="!checkDisabled(listSelected,item)">
          ...
          <el-upload-pack
            :item="item"
            @changePercent="$emit('changePercent')"
            @responseResource="voteResource(item, arguments[0])"
          ></el-upload-pack>
          <el-tag :type="item.resourceId?'success':'warning'">{{item.resourceId?'已上传':'未上传'}}</el-tag>
        </template>
      </div>
      <!--end-->
    </template>
...
<script>
import ElUploadPack from '@/components/el-upload-pack'
export default {
  name: 'normal-condition',
  components: {
    'el-upload-pack': ElUploadPack
  },
  ...
  methods: {
    ...
    // 固定玩家资格配置 资源上传结果响应
    fixedResource(item, uploadId) {
      item.importId = uploadId
      // console.log(uploadId,resourceId)
    },
    // 投票 资源上传结果响应
    voteResource(item, uploadId) {
      item.resourceId = uploadId
      // console.log(uploadId,resourceId)
    },
    downTemplate(id) {
      if (id == 'ce59f4f3-75c9-472b-bfe7-0280405df206') {
        //固定玩家资格配置

      } else if (id == 'a4ded85d-69e3-4132-90d3-59dadc98312f') {
        window.open('/static/template/投票上传模板.xlsx') // 投票模板下载
      }
    }
  },
  watch: {
